...
//Note we will use "hello_backend" in this JavaScript code a few times to call the backend
...
document.addEventListener('DOMContentLoaded', async (e) => {
  ...
  const question = await hello_backend.getQuestion();
  ...
  const voteCounts = await hello_backend.getVotes();
...
//via "await hello_backend.vote(selectedOption)"
pollForm.addEventListener('submit', async (e) => {
  ...
  const updatedVoteCounts = await hello_backend.vote(checkedValue);
...
resetButton.addEventListener('click', async (e) => {
  ...
  await hello_backend.resetVotes();
  const voteCounts = await hello_backend.getVotes();
  updateLocalVoteCounts(voteCounts);
